...
#ifndef _GMD_BINARY_TREE_SPLAY_
#define _GMD_BINARY_TREE_SPLAY_

template <typename Key, typename Value, typename Info, bool SetMap, bool Threaded>
struct binary_tree_node<tree_splay, Key, Value, Info, SetMap, Threaded>
: public binary_tree_node_base<binary_tree_node<tree_splay, Key, Value, Info, SetMap, Threaded>,
                               Key, Value, Info, SetMap, Threaded>
{
	...
	private:
	using _Node  = binary_tree_node<tree_splay, Key, Value, Info, SetMap, Threaded>;
	using _Base  = binary_tree_node_base<_Node, Key, Value, Info, SetMap, Threaded>;
	using _Key   = Key;
	using _Value = Value;
	using _Info  = Info;

	static constexpr binary_tree_type _Tree = tree_splay;
...
struct binary_tree_subbase<tree_splay, Node, Multi, Comparator, Allocator>
: public binary_tree_base<Node, Multi, Comparator, Allocator>
{
	...
	{
		if(!Multi) {
			_Node *node = _Base::_find(key); if(node != &(_Base::_head)) { _splay(node); return 1; } return 0; }
		else {
			size_t count = 0;
			_Node *lower = _Base::_lower_bound(key), *upper = _Base::_upper_bound(key);
			for( ; lower != upper; lower = _Iteration::_<1>(lower), ++count) _splay(lower);
			return count; }
	}
	/* === Count (override) === */
	...
	{ _Node *node = _Base::_find(key); if(node != &(_Base::_head)) { _splay(node); return true; } return false; }
	/* === Contains (override) === */
	...
	find (const Key &key)
	{ _Node *node = _Base::_find(key); if(node != &(_Base::_head)) _splay(node); return T(node); }
	/* === Find (override) === */
	...
	{ _Node *node = _Base::_find_short(key); if(node != &(_Base::_head)) _splay(node); return T(node); }
	/* === Find short (override) === */
	...
	{ _Node *node = _Base::_lower_bound(key); if(node != &(_Base::_head)) _splay(node); return T(node); }
	/* === Lower bound (override) === */
	...
	{ _Node *node = _Base::_upper_bound(key); if(node != &(_Base::_head)) _splay(node); return T(node); }
	/* === Upper bound (override) === */
	...
	equal_range (const Key &key)
	{
		...
		for(_Node *node = lower; node != upper; node = _Iteration::_<1>(node)) _splay(node);
	...
	_insert_ (Arg &&info)
	{
		...
		_splay(result.first); return result;
	}
